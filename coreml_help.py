...
class CoremlBrowser(object):
  ...
  def get_shapes(self, mlmodelc: upath) -> dict:
    ...
    Returns:
      If successful a valid shape dictionary
    ...
    try to import and save shape info for the model. Grabs all the
  ...
  def init_shapes(self, mlmodelc:upath=None, prefer_shaper=True):
    """
    Get the NN Shaper object, or if that fails, try to get the shape dictionary.
    ...
    If the `NeuralNetworkShaper` object throws an exception
    (prob. because the network is invalid in some way),
    try to import and save shape info for the model from the compiled bundle - i.e. '.mlmodelc'.
    Grabs all the
    shape info and saves it in a dictionary. Possible issues:
     - CoreML Compiler may not be available
     - Its static - if the model is changed, this has to be be run again
    ...
    if self.shaper is None and mlmodelc is not None:
      self.shape_dict = self.get_shapes(mlmodelc)

    if self.shape_dict is not None:
      print(f"Using shape dictionary from {mlmodelc / 'model.espresso.shape'}")

    if self.shaper is None and self.shape_dict is None:
      print("Can't infer shapes because 'NeuralNetworkShaper' is not available")
      print("And compiled ('.mlmodelc') version of model was not specified or is not available ")
